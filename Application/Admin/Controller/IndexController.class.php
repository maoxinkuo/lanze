...
class IndexController extends BackendController {
    ...
    public function login() {
        if (IS_POST) {
            ...
            if(!$err){
                ...
                $this->apply['Subsite'] && $admin['subsite'] && $session['subsite'] = explode(',',$admin['subsite']);
    ...
    public function total(){
        ...
        $total['yesterday_jobs'] = intval($total['jobs']) + intval($total['jobsTemp']);
        ...
        //待处理事项
        $total['jobs_audit'] = M('JobsTmp')->where(array('audit'=>2))->count('id');
        ...
        $total['resume_img_audit'] = M('ResumeImg')->where(array('photo_audit'=>2))->count('id');
