...
class PersonalController extends BackendController{
    ...
    public function index(){
        ...
        if($key_type && $key){
            ...
            $audit = $tabletype == 1 ? 0 : I('request.audit','','intval');
        ...
        $this->where = $where;
        $this->join = $db_pre."members_info as i on ".$this_t.".uid=i.uid";
    ...
    public function member_delete(){
        ...
        if (I('post.delete_resume')=='yes' && false===D('Resume')->admin_del_resume_for_uid($tuid))
        {
            $this->error('删除简历失败！');
        }
    ...
    public function promotion_add(){
        ...
        if(IS_POST){
            ...
            $_POST['endtime'] = strtotime(intval($days)." day");
        }else{
            $this->_name = 'PersonalServiceStickLog';
            $this->assign('tag_list',$tag_list);
        }
