...
class CompanyMembersController extends BackendController{
    ...
    public function user_setmeal_set(){
        ...
        if($rst['state']==1){
            ...
            $notes="操作人：".C('visitor.username').",说明：为会员 {$userinfo['username']} 重新开通服务，收取服务金额：{$amount}元，服务ID：{$reg_service}。";
            $members_charge_log['_t']='MembersChargeLog';
            $members_charge_log['log_uid']=$userinfo['uid'];
            $members_charge_log['log_username']=$userinfo['username'];
            $members_charge_log['log_type']=4;
    ...
    public function delete(){
        ...
        if (I('post.delete_company')=='yes' && false===D('CompanyProfile')->admin_delete_company($tuid))
        {
            $this->error('删除企业资料失败！');
        }
        if (I('post.delete_jobs')=='yes' && false===D('Jobs')->admin_delete_jobs_for_uid($tuid))
        {
            $this->error('删除职位失败！');
        }
