...
class CategoryModel extends Model
{
    ...
    public function category_wage(){
        $categoryData = $this->where(array('c_alias'=>'QS_wage'))->order('c_order asc')->getfield('c_id,c_name');
    ...
    public function category_cache() {
        $category = array();
        $categoryData = $this->field('c_id,c_alias,c_name')->order('c_order asc')->select();
        ...
        foreach ($category['QS_wage'] as $key => $val) {
            if(preg_match_all('(\d+)',$val,$reg)){
                $reg = $reg[0];
                $minwage = $reg[0]%1000==0?(($reg[0]/1000).'K'):(round($reg[0]/1000,1).'K');
                $maxwage = $reg[1]?($reg[1]%1000==0?(($reg[1]/1000).'K'):(round($reg[1]/1000,1).'K')):'';
