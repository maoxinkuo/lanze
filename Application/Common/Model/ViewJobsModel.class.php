	...
	*/
	public function get_view_jobs($data,$pagesize=10)
	{
		...
		if(C('visitor.utype')==2)//个人查看
		{
			...
			$rst['list'] = $this->where($where)->join($join)->join($join1)->field($this_t.'.*,j.tdistrict,j.sdistrict,j.district,j.jobs_name,j.uid as company_uid,j.audit,j.companyname,j.company_id,j.deadline,j.display')->order('id desc')->limit($pager->firstRow . ',' . $pager->listRows)->select();
			if(C('apply.Subsite')){
		...
		else//企业查看
		{
			...
			$where['i.def'] = 1;
			$uids = $this->where($where)->join($join)->group($this_t.'.uid')->getfield($this_t.'.uid');
			$count = count($uids);
			$pager =  pager($count, $pagesize);
