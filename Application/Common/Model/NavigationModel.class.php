...
class NavigationModel extends Model{
	...
	public function nav_cache(){
		$nav_data = $this->field('alias,title,pagealias,tag,target,color,list_id,url')->where(array('display'=>1))->order('navigationorder')->select();
	...
	public function personal_nav_cache(){
		$personal_nav_list = $this->field('title,pagealias,tag,target,color,list_id,url')->where(array('alias'=>'QS_top','is_personal'=>1,'display'=>1))->order('navigationorder')->select();
