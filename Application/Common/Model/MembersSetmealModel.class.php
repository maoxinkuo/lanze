	...
	public function set_members_setmeal($uid,$id)
	{
		...
		else
		{
			if(false === $this->where(array('uid'=>$uid))->save())
			{
				...
				M('Jobs')->where(array('uid'=>$uid))->save($setmeal_jobs);
				M('JobsTmp')->where(array('uid'=>$uid))->save($setmeal_jobs);
				D('Jobs')->distribution_jobs_uid($uid);
				M('JobsSearch')->where(array('uid'=>$uid))->setfield('setmeal_id',$setmeal_jobs['setmeal_id']);
				M('JobsSearchKey')->where(array('uid'=>$uid))->setfield('setmeal_id',$setmeal_jobs['setmeal_id']);
