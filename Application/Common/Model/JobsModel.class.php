...
class JobsModel extends Model{
	...
	public function get_jobs($where,$orderby,$tabletype,$pagesize=10,$countresume=false,$promotion=false,$auto_refresh=false,$views=false){
		if($tabletype=="all"){
			if($pagesize!='-1'){
				...
				$union_sql = M('JobsTmp')->where($where)->order($orderby)->limit($pager->firstRow . ',' . $pager->listRows)->buildSql();
			}else{
				$union_sql = M('JobsTmp')->where($where)->order($orderby)->buildSql();
			}
			$list = M('Jobs')->union($union_sql,true)->where($where)->select();
		}else{
			if($pagesize!='-1'){
	...
	*/
	public function distribution_jobs($id,$uid=false){
		...
		foreach($id as $v){
			$where['id']=intval($v);
			$t1 = $this->where($where)->find();
			$t2 = M('JobsTmp')->where($where)->find();
			if ((!$t1 && !$t2) || ($t1 && $t2)){
				continue;
			}else{
				$j = $t1?$t1:$t2;
	...
	{
		$uid=intval($uid);
		$union_count_sql = M('JobsTmp')->where(array('uid'=>$uid))->field('id')->buildSql();
		$id = $this->union($union_count_sql,true)->where(array('uid'=>$uid))->getField('id',true);
    ...
    */
	...
	public function jobs_refresh($data)
	{
		...
		$data_save = array('refreshtime'=>$time,'stime'=>$time);
		...
		if (fieldRegex($sqlin,'in'))
		{
			...
			if ($audit=="1" && $mailconfig['set_jobsallow']=="1")//审核通过
			{
				...
				foreach ($result as $key => $value) {
					...
					if($useremail['email']){
						D('Mailconfig')->send_mail(array('sendto_email'=>$useremail['email'],'send_type'=>'set_jobsallow'));
					}
				}
			}
			if ($audit=="3" && $mailconfig['set_jobsnotallow']=="1")//审核未通过
			{
				$result = M('JobsTmp')->where(array('id'=>array('in',$sqlin)))->select();
				foreach ($result as $key => $value) {
					$useremail = D('Members')->get_user_one(array('uid'=>$value['uid']));
					if($useremail['email']){
						D('Mailconfig')->send_mail(array('sendto_email'=>$useremail['email'],'send_type'=>'set_jobsnotallow'));
					}
				}
			}
			//sms
			if ($audit=="1" && C('qscms_sms_open')==1 && $sms['set_jobsallow']=="1" )
			{
				...
				foreach ($result as $key => $value) {
					...
					{
						D('Sms')->sendSms('notice',array('mobile'=>$usermobile['mobile'],'tpl'=>'set_jobsallow','data'=>array('jobsname'=>$value['jobsname'])));
					}
				}
			}
			//sms
			if ($audit=="3" && C('qscms_sms_open')==1 && $sms['set_jobsnotallow']=="1" )//认证未通过
			{
				...
				foreach ($result as $key => $value) {
					...
					if ($usermobile['mobile_audit']=="1" && !is_array($value['mobile'],$mobilearray))
					{
						D('Sms')->sendSms('notice',array('mobile'=>$usermobile['mobile'],'tpl'=>'set_jobsnotallow','data'=>array('jobsname'=>$value['jobsname'])));
					}
				}
			}
			//微信通知
			...
			if($module_list['Weixin']){
				if($audit=="1")
				{
					...
					$jobs_list = D('Jobs')->where(array('id'=>array('in',$sqlin)))->union($union)->select();
