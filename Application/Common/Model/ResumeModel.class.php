...
class ResumeModel extends RelationModel{
	...
	protected $_validate = array(
		...
		array('uid,fullname,residence,nature,birthdate,experience,wage,education,current','identicalNull','',0,'callback'),
		...
		array('residence','2,30','{%resume_residence_length_error}',0,'length'), // 现居住地
		array('householdaddress','2,60','{%resume_householdaddress_length_error}',2,'length'), // 户口所在地
		array('specialty','0,200','{%resume_specialty_length_error}',0,'length'), // 自我描述
	...
	*/
	public function add_resume($data,$user){
		...
		if(false === $d = $this->create($data)){
            ...
            $this->wage_cn         = $d['wage_cn']            = $category['QS_wage'][$d['wage']];
	...
	*/
	public function check_resume($uid,$pid)
	{
		...
		/* 分词 start */
		$setsqlarr['key_full'] = $setsqlarr['key_precise'] = $resume['intention_jobs'].$resume['specialty'].$resume['education_cn'];
		...
		if (!empty($resume_work)){
			foreach($resume_work as $li){
				$setsqlarr['key_full'].=$li['companyname'].$li['speciality'];
			}
			$setsqlarr['key_precise'].=$resume_education[0]['companyname'].$resume_education[0]['speciality'];
		}
		if (!empty($resume_training)){
			foreach($resume_training as $li){
				$setsqlarr['key_full'].=$li['agency'].$li['speciality'];
			}
			$setsqlarr['key_precise'].=$resume_education[0]['agency'].$resume_education[0]['speciality'];
		}
	...
	public function get_outward_resumes_tpl($uid,$resume_id)
	{
		...
		$html = $controller->assign_resume_tpl(array('resume_basic'=>$resume_basic,'resume_work'=>$resume_work),'Emailtpl/outward_resume');
		return $html;
	}
	...
	public function admin_edit_resume_audit($id,$audit,$reason,$pms_notice,$audit_man='')
	{
		...
		{
			$resume_list = $this->field('id,uid,display,audit,stime,refreshtime,key_full,key_precise')->where(array('id'=>array('in',$sqlin)))->select();
			...
			//填写管理员日志
			admin_write_log("修改简历id为".$sqlin."的审核状态为".$audit, C('visitor.username'),3);
			//发送站内信
			if ($pms_notice=='1')
			{
				$result = $this->where(array('id'=>array('in',$sqlin)))->field('fullname,title,uid')->select();
				$reason=$reason==''?'未知':$reason;
				if($result){
					foreach ($result as $key => $value) {
						$user_info=D('MembersInfo')->get_userprofile(array('uid'=>$value['uid']));
						$setsqlarr['message']=$audit=='1'?"您创建的简历：{$value['title']},真实姓名：{$value['fullname']},成功通过网站管理员审核！":"您创建的简历：{$value['title']},真实姓名：{$value['fullname']},未通过网站管理员审核,{$reason}";
						$setsqlarr['msgtype']=1;
						$setsqlarr['msgtouid']=$user_info['uid'];
						$setsqlarr['msgtoname']=$user_info['realname'];
						$setsqlarr['dateline']=time();
						$setsqlarr['replytime']=time();
						$setsqlarr['new']=1;
						D('Pms')->data($setsqlarr)->add();
					}
				}
			}
			...
			if ($audit=="1" && $mailconfig['set_resumeallow']=="1")//审核通过
			{
				...
				foreach ($result as $key => $value) {
					...
					if($useremail['email']){
						D('Mailconfig')->send_mail(array('sendto_email'=>$useremail['email'],'send_type'=>'set_resumeallow'));
					}
				}
			}
			if ($audit=="3" && $mailconfig['set_resumenotallow']=="1")//审核未通过
			{
				...
				foreach ($result as $key => $value) {
					$useremail = D('Members')->get_user_one(array('uid'=>$value['uid']));
					if($useremail['email']){
						D('Mailconfig')->send_mail(array('sendto_email'=>$useremail['email'],'send_type'=>'set_resumenotallow'));
					}
				}
			}
			//sms
			...
			if($module_list['Weixin']){
				if($audit=="1")
				{
					$resume_list = D('Resume')->where(array('id'=>array('in',$sqlin)))->select();
	...
	public function send_to_email_one($resume_id,$email){
		$resume_basic = $this->get_resume_one($resume_id);
		if($resume_basic['tag_cn'])
		{
			$resume_basic['tag_arr']=explode(',',$resume_basic['tag_cn']);
		}else{
