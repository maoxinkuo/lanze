...
class PersonalJobsSubscribeModel extends Model{
	protected $_validate = array(
		array('intention_jobs_id','identicalNull','',1,'callback',3),
		array('intention_jobs_id,trade,district,wage','identicalEnum','',2,'callback'),
	...
	public function edit_subscribe($user,$id){
		if(false !== $this->create()){
			...
			foreach(explode(',',$this->district) as $val) {
				$val = explode('.',$val);
				$district[] = $val[2] ? $city[$val[1]][$val[2]] : ($val[1] ? $city[$val[0]][$val[1]] : $city[0][$val[0]]);
			}
			$this->district_cn = implode(',',$district);
			...
			if(false !== $id = $this->$name()){
	            //写入会员日志
	            write_members_log($user,2039,$id);
