...
class CategoryJobsModel extends Model
{
	...
	public function custom_jobs_cache(){
		$jobs = array();
		$jobsData = $this->field('id,parentid,categoryname,spell')->order('category_order asc')->select();
	...
	public function jobs_cache(){
		$jobs = array();
		$jobsData = $this->field('id,parentid,categoryname')->order('category_order asc')->select();
		foreach ($jobsData as $key => $val) {
	...
	public function jobs_cate_cache(){
		$jobsSpell = $this->order('parentid asc')->getfield('spell,id,parentid,categoryname');
