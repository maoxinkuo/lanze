...
class SubsiteModel extends Model
{
	protected $_validate = array(
		array('s_sitename,s_domain,s_tpl,s_district','identicalNull','',0,'callback'),
	...
	public function subsite_cache(){
		$data = $this->where(array('s_effective'=>1))->order('s_order')->getfield('s_id,s_domain,s_m_domain,s_sitename,s_district,s_districtname,s_logo_home,s_logo_user,s_logo_other,s_title,s_keywords,s_description,s_level');
	...
	public function subsite_district_cache(){
		$data = $this->field('s_id,s_district')->where(array('s_effective'=>1))->order('s_order')->select();
		foreach ($data as $key => $val) {
	...
	public function get_subsite_domain(){
		if(false === $subsite = F('subsite_domain_list')){
			$subsite_list = $this->where(array('s_effective'=>1))->order('s_ordid')->getfield('s_id,s_sitename,s_domain,s_district,s_districtname,s_m_domain,s_tpl,s_spell,s_ordid');
