...
class company_jobs_listTag {
    ...
    public function run(){
        ...
        if($company_info){
            ...
            $company = M('CompanyProfile')->where(array('id'=>array('in',$cids)))->order('field(id,'.implode(',',$cids).')')->getfield('id,uid,audit,companyname,nature_cn,district_cn,addtime,refreshtime,logo,famous');
            $cids = array();
