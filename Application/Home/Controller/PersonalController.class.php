...
class PersonalController extends FrontendController{
    ...
    */
    ...
    public function refresh_resume(){
        if(IS_AJAX){
            ...
            !$pid && $pid = M('Resume')->where(array('uid'=>C('visitor.uid')))->order('def desc')->getField('id');
            $uid = C('visitor.uid');
            ...
            if(C('qscms_per_refresh_resume_time')!=0&&($count>=C('qscms_per_refresh_resume_time'))){
                $this->ajaxReturn(0,"每天最多只能刷新".C('qscms_per_refresh_resume_time')."次,您今天已超过最大刷新次数限制！");
            }elseif($duringtime<=$space && $space!=0){
                $this->ajaxReturn(0,C('qscms_per_refresh_resume_space')."分钟内不能重复刷新简历！");
            }else{
                //修改目前状态
                $resume = D('Resume');
    ...
    */
    public function resume_add(){
        ...
        if(IS_POST && IS_AJAX){
            ...
            $trims = array('telephone','title','fullname','residence','email','householdaddress','intention_jobs','intention_jobs_id','trade','district');
            foreach ($ints as $val) {
    ...
    */
    ...
    public function ajax_save_basic_info(){
        ...
        $trims = array('telephone','fullname','residence','email','householdaddress');
        foreach ($ints as $val) {
            $setsqlarr[$val] = I('post.'.$val,0,'intval');
        }
        foreach ($trims as $val) {
    ...
    */
    ...
    public function save_education(){
        ...
        if ($setsqlarr['todate'] == 1) {
            ...
            if($setsqlarr['startyear'] == intval(date('Y')) && $setsqlarr['startmonth'] >= intval(date('m'))) $this->ajaxReturn(0,'就读开始月份不允许大于毕业时间!');
        } else {
            ...
            if($setsqlarr['startyear'] == $setsqlarr['endyear'] && $setsqlarr['startmonth'] >= $setsqlarr['endmonth']) $this->ajaxReturn(0,'就读开始月份不允许大于毕业时间!');
        }
    ...
    public function save_work(){
        ...
        if ($setsqlarr['todate'] == 1) {
            ...
            if($setsqlarr['startyear'] > intval(date('Y'))) $this->ajaxReturn(0,'工作开始时间不允许大于毕业时间！');
            if($setsqlarr['startyear'] == intval(date('Y')) && $setsqlarr['startmonth'] >= intval(date('m'))) $this->ajaxReturn(0,'工作开始月份不允许大于毕业时间！');
        } else {
            if(!$setsqlarr['startyear'] || !$setsqlarr['startmonth'] || !$setsqlarr['endyear'] || !$setsqlarr['endmonth']) $this->ajaxReturn(0,'请选择工作时间！');
            if($setsqlarr['startyear'] > $setsqlarr['endyear']) $this->ajaxReturn(0,'工作开始时间不允许大于毕业时间');
            if($setsqlarr['startyear'] == $setsqlarr['endyear'] && $setsqlarr['startmonth'] >= $setsqlarr['endmonth']) $this->ajaxReturn(0,'工作开始月份不允许大于毕业时间！');

            if($setsqlarr['endyear'] > intval(date('Y'))) $this->ajaxReturn(0,'工作结束时间不允许大于当前时间');
            if($setsqlarr['endyear'] == intval(date('Y')) && $setsqlarr['endmonth'] > intval(date('m'))) $this->ajaxReturn(0,'工作开始月份不允许大于毕业时间！');
        }
        if(false === $resume = $this->_is_resume()) $this->ajaxReturn(0,'请先填写简历基本信息！');
        $setsqlarr['pid'] = $resume['id'];
    ...
    public function save_training(){
        ...
        if ($setsqlarr['todate'] == 1) {
            ...
            if($setsqlarr['startyear'] == intval(date('Y')) && $setsqlarr['startmonth'] >= intval(date('m'))) $this->ajaxReturn(0,'培训开始月份不允许大于毕业时间！');
        } else {
            ...
            if($setsqlarr['startyear'] == $setsqlarr['endyear'] && $setsqlarr['startmonth'] >= $setsqlarr['endmonth']) $this->ajaxReturn(0,'培训开始月份不允许大于毕业时间！');
        }
        if(false === $resume = $this->_is_resume()) $this->ajaxReturn(0,'请先填写简历基本信息！');
        $setsqlarr['pid'] = $resume['id'];
    ...
    */
    ...
    public function resume_outward(){
        $resume_list=M('Resume')->field('id,title')->where(array('uid'=>C('visitor.uid')))->select();
        $this->assign('resume_list',$resume_list);
    ...
    */
    ...
    public function del_favorites(){
        if(IS_AJAX){
            ...
            if($reg['state']===true){
                $this->success("删除成功！",'jobs_favorites');
            }else{
    ...
    public function user_info(){
        ...
        if(IS_POST){
            ...
            $trims = array('phone','email','realname','residence','householdaddress');
            foreach ($ints as $val) {
                $setsqlarr[$val] = I('post.'.$val,0,'intval');
            }
            foreach ($trims as $val) {
