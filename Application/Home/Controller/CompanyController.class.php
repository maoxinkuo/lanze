...
class CompanyController extends FrontendController{
    ...
    public function index(){
		...
		$this->assign('total_view',M('ViewJobs')->where(array('jobsid'=>array('in'=>$jids)))->count());
    ...
    public function jobs_list(){
        ...
        $jobs_list = D('Jobs')->get_jobs(array('uid'=>C('visitor.uid')),'refreshtime desc',$tabletype,10,true,true,true);
    ...
    public function jobs_add()
    {
		...
		if(IS_POST){
            ...
            $setsqlarr['famous']=$this->company_profile['famous'];
            if(!$id){
                $setsqlarr['audit']= 2;
            }else{
    ...
    */
	...
	public function jobs_interview(){
        ...
        $jobs_list1 = M('Jobs')->where(array('uid'=>C('visitor.uid')))->getField('id,jobs_name');
    ...
    */
	...
	public function jobs_apply(){
        ...
        $jobs_list = M('Jobs')->where(array('uid'=>C('visitor.uid')))->getField('id,jobs_name');
        if($stop){
	...
	*/
	...
	public function jobfair_list(){
		...
		$jobfair = D('Jobfair/Jobfair')->get_lately_jobfair_list(C('visitor'));
	...
	*/
	...
	public function company_tpl(){
        ...
        //未购买模板
        if(!empty($tplid_arr)){
        	$def_tplid_list = D('Tpl')->where(array('tpl_type'=>1,'tpl_id'=>array('not in',$tplid_arr)))->select();
        }else{
        	$def_tplid_list = D('Tpl')->where(array('tpl_type'=>1))->select();
        }
