...
class CompanyServiceController extends CompanyController{
    ...
    public function jobs_refresh_all(){
    	...
    	if($user_jobs+$refresh_time>$this->my_setmeal['refresh_jobs_free']){
    		$this->ajaxReturn(2,'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您当前共有 <span class="font_yellow">'.$user_jobs.'</span> 条在招职位，今天免费刷新次数已用完，请前往 <a target="_blank" href="'.U('company/jobs_list').'" class="font_blue">职位列表</a> 单条刷新。',U('company/jobs_list'));
    	}else{
    		$jobsid_arr = D('Jobs')->where(array('uid'=>C('visitor.uid')))->field('id')->select();
    		$yid = array();
