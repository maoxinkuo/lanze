...
1.绑定您的".C('qscms_site_name')."帐号，求职招聘更加方便，并实时接收提醒通知。<a href='".$this->get_url(U('mobile/Members/apilogin_binding',array('openid'=>$object->FromUserName)))."'>点此立即绑定</a>
2.您可以回复【j】或【n】获取紧急、最新招聘信息，或者回复职位、公司等关键词如【销售】获取相关职位信息。
3.每日签到，免费获取".C('qscms_points_byname')."，商城好礼兑不停！";
                break;
            case "LOCATION":
                  $map=(array)$object;
                  S('location_'.$object->FromUserName,$map);
	            break;
            case "SCAN":
	...
	protected function actionScan($object){
		...
		if($event_key<=10000000){
			...
			if($usinfo){
				...
				$this->content = "<a href='".C('qscms_site_domain').'?m=Home&c=Callback&a=weixin_login&openid='.$object->FromUserName.'&uid='.$usinfo['uid'].'&event_key='.$event_key."'>点此立即登录".C('qscms_site_name')."网页</a>";
			}else{
				$this->content="您还未绑定".C('qscms_site_name')."帐号，现在开始绑定：<a href='".$this->get_url(U('mobile/Members/apilogin_binding',array('openid'=>$object->FromUserName)))."'>点击开始注册/绑定帐号</a>";
			}
			...
			$this->content="<a href='".$this->get_url(U('mobile/Members/apilogin_binding',array('openid'=>$object->FromUserName)))."'>点击此处开始注册</a>";
	...
	protected function clickNearbyJobs($object){
		...
		if($usinfo){
			if($usinfo['utype']!=2){
				...
				$data = S('location_'.$object->FromUserName);
				...
				if(!$jobs_list['list']){
					...
					foreach ($jobs_list['list'] as $key => $value){
					    ...
					    $url = $this->get_url(U('mobile/jobs/show',array('id'=>$value['id'],'openid'=>$object->FromUserName)));
					    if($i==1){
		                    $picurl = $first_pic;
						}else{
...
您目前获得的总奖励为:".$all_points.C('qscms_points_byname');
			...
			if($ids){
				...
				foreach ($jobs_list as $key => $val) {
				    ...
				    $url = $this->get_url(U('mobile/jobs/show',array('id'=>$val['id'],'openid'=>$object->FromUserName)));
				    if($i==1){
	                    $picurl=$first_pic;	
					}else{
