...
!function($) {
    ...
    var handler = function(captchaObj) {
        ...
        captchaObj.onError(function() {
            var loginTypeValue = eval($('#J_loginType').val());
            $('.type_box').eq(loginTypeValue).addClass('err');
            $('.type_box').eq(loginTypeValue).find('.J_errbox').text('网络错误，请稍候再试！');
        });
	...
	$('#J_do_login_btn_sub').click(function() {
    	...
    	if (usernameValue == '') {
            $thisTypeBox.addClass('err');
            $thisTypeBox.find('.J_errbox').text('请输入您的姓名！');
    		return false;
    	}
    	if (passwordValue == '') {
            $thisTypeBox.addClass('err');
            $thisTypeBox.find('.J_errbox').text('请输入您的电话！');
    		return false;
    	}
    
    	
        $('#J_do_login_btn').val('保存中...').addClass('btn_disabled').prop('disabled', !0);
			
			dosub(usernameValue,passwordValue);
		
        
    });

	function dosub(usernameValue,passwordValue){
	     $.ajax({
                url: qscms.root+'?m=Home&c=Members&a=submit',
                type: "post",
                dataType: "json",
                data: {
                    name: usernameValue,
